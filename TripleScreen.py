...
for i in range(1, len(df.close)):
    if df.ema130.values[i-1] < df.ema130.values[i] and \
        df.slow_d.values[i-1] >= 20 and df.slow_d.values[i] < 20: #1.130일 이동 지수평균이 상승하고%D가 20아래로 떨어지면
        plt.plot(df.number.values[i], 250000, 'r^')               #2.빨간색 삼각형으로 매수 신호를 표시한다
    elif df.ema130.values[i-1] > df.ema130.values[i] and \
        df.slow_d.values[i-1] <= 80 and df.slow_d.values[i] > 80: #3.130일 이동 지수평균이 하락하고%D가 80위로 상승하면
        plt.plot(df.number.values[i], 250000, 'bv')               #4.파란색 삼각형으로 매수신호를 표시한다
plt.legend(loc='best')
